// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.14.0
// source: truck.sql

package generated

import (
	"context"
	"database/sql"
	"time"

	"github.com/google/uuid"
)

const addTruck = `-- name: AddTruck :exec
INSERT INTO truck (id, created_at, winery_id, name)
VALUES ($1, $2, $3, $4)
`

type AddTruckParams struct {
	ID        uuid.UUID `db:"id"`
	CreatedAt time.Time `db:"created_at"`
	WineryID  uuid.UUID `db:"winery_id"`
	Name      string    `db:"name"`
}

func (q *Queries) AddTruck(ctx context.Context, arg AddTruckParams) error {
	_, err := q.db.Exec(ctx, addTruck,
		arg.ID,
		arg.CreatedAt,
		arg.WineryID,
		arg.Name,
	)
	return err
}

const getTruck = `-- name: GetTruck :one
SELECT id, created_at, updated_at, deleted_at, winery_id, name
FROM truck
WHERE id = $1
LIMIT 1
`

func (q *Queries) GetTruck(ctx context.Context, id uuid.UUID) (Truck, error) {
	row := q.db.QueryRow(ctx, getTruck, id)
	var i Truck
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.DeletedAt,
		&i.WineryID,
		&i.Name,
	)
	return i, err
}

const listTrucks = `-- name: ListTrucks :many
SELECT id, created_at, updated_at, deleted_at, winery_id, name
FROM truck
ORDER BY created_at DESC
OFFSET $1 LIMIT $2
`

type ListTrucksParams struct {
	Offset int32 `db:"offset"`
	Limit  int32 `db:"limit"`
}

func (q *Queries) ListTrucks(ctx context.Context, arg ListTrucksParams) ([]Truck, error) {
	rows, err := q.db.Query(ctx, listTrucks, arg.Offset, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Truck
	for rows.Next() {
		var i Truck
		if err := rows.Scan(
			&i.ID,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.WineryID,
			&i.Name,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const updateTruckData = `-- name: UpdateTruckData :exec
UPDATE truck
SET name       = $2,
    updated_at = $3
WHERE id = $1
`

type UpdateTruckDataParams struct {
	ID        uuid.UUID    `db:"id"`
	Name      string       `db:"name"`
	UpdatedAt sql.NullTime `db:"updated_at"`
}

func (q *Queries) UpdateTruckData(ctx context.Context, arg UpdateTruckDataParams) error {
	_, err := q.db.Exec(ctx, updateTruckData, arg.ID, arg.Name, arg.UpdatedAt)
	return err
}
